...
$localDb = new LocalDb();
if(   isset($_REQUEST['meter'])
    &isset($_REQUEST['volume'])
    &isset($_REQUEST['billRate'])
    &isset($_REQUEST['total'])){
        $meter=$_REQUEST['meter'];
        $volume=$_REQUEST['volume'];
        $bill_rate=$_REQUEST['billRate'];
        $totalCost=$_REQUEST['totalCost'];
        
    $sql="insert into readings set meter='$meter', volume='$volume',
    billRate='$bill_rate',totalCost='$totalCost'";
	...
	$sqlRequest="select * from readings";
	$localDb->query($sqlRequest);
